...
<html>    
  <head>  
	...
	<script type="text/javascript">    
		function add(){      
			Dialog.open({Title:"新增", Width:1000, Height:490, URL:"<%=path%>/Product!toAddProduct.action"});   
		}      
		function mod(){      
			...
			Dialog.open({Title:"编辑", Width:1000, Height:490, URL:"<%=path%>/Product!toAddProduct.action?oper=1&productBean.id="+id});
		}      
		function detail(){   
			var id = getCheckRowValue();     
			if(id==null) {     
			...
			Dialog.open({Title:"详情", Width:500, Height:360, URL:"<%=path%>/Product!toAddProduct.action?read=1&oper=1&productBean.id="+id});    
		}      
		function del(){      
			if(!isCheckAny("idArr")){      
			...
			Dialog.confirm("确定删除吗", function(){     
				...
				form1.action = "<%=path%>/Product!delProducts.action";
				form1.target = "fram";
				form1.submit();  
		...
		function search() {  
			Dialog.open({Title:"查询", Width:650, Height:220, URL:"<%=path%>/Product!search.action",OnLoad:function(){
  ...
  <body>  
  <GF:BodyCaption label="产品" ico="images/ico/user.gif"> 
  	<GF:ToolBar id="123">     
    ...
    <form action="<%=path%>/Product!list.action" name="form1" id="form1" method="get">  
   
			<input type="hidden" name="productBean.name" id="name" value="${productBean.name}"/> 
			<input type="hidden" name="productBean.parentId" id="parentId" value="${productBean.parentId}"/> 
			<input type="hidden" name="productBean.description" id="description" value="${productBean.description}"/> 
			...
			<table id="table1" width="100%" height="100%"  border="0" cellspacing="0" cellpadding="0"> 
			   	 <thead>
			   		<tr>  
			   			...
			   			<th sortname="parent_id" width="10%">父级id</th>	   
