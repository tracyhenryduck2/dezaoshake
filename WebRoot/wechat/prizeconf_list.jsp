...
<html>    
  <head>  
	...
	<script type="text/javascript">    
		function add(){      
			Dialog.open({Title:"新增", Width:600, Height:360, URL:"<%=path%>/wechat/Prizeconf!toAddPrizeconf.action"});   
		}      
		function mod(){      
			...
			Dialog.open({Title:"编辑", Width:600, Height:360, URL:"<%=path%>/wechat/Prizeconf!toAddPrizeconf.action?oper=1&prizeconfBean.id="+id});
		}      
		function detail(){   
			var id = getCheckRowValue();     
			if(id==null) {     
			...
			Dialog.open({Title:"详情", Width:500, Height:360, URL:"<%=path%>/wechat/Prizeconf!toAddPrizeconf.action?read=1&oper=1&prizeconfBean.id="+id});    
		}      
		function del(){      
			if(!isCheckAny("idArr")){      
			...
			Dialog.confirm("确定删除吗", function(){     
				...
				form1.action = "<%=path%>/wechat/Prizeconf!delPrizeconfs.action";
				form1.target = "fram";
				form1.submit();  
		...
		function search() {  
			Dialog.open({Title:"查询", Width:650, Height:220, URL:"<%=path%>/wechat/Prizeconf!search.action",OnLoad:function(){
			     this.innerDoc.getElementById("name").value = $("#name").val();
			     this.innerDoc.getElementById("sum").value = $("#sum").val();
			     this.innerDoc.getElementById("p").value = $("#p").val();
           }
			});    
		}      
  ...
  <body>  
  <GF:BodyCaption label="奖项设置" ico="images/ico/user.gif"> 
  	<GF:ToolBar id="123">     
    ...
    <form action="<%=path%>/wechat/Prizeconf!list.action" name="form1" id="form1" method="get">  
   
			<input type="hidden" name="prizeconfBean.name" id="name" value="${prizeconfBean.name}"/> 
			<input type="hidden" name="prizeconfBean.sum" id="sum" value="${prizeconfBean.sum}"/> 
			<input type="hidden" name="prizeconfBean.p" id="p" value="${prizeconfBean.p}"/> 
			...
			<table id="table1" width="100%" height="100%"  border="0" cellspacing="0" cellpadding="0"> 
			   	 <thead>
			   		<tr>  
			   			...
			   			<th sortname="name" width="10%">奖项名称</th>	   
			   			<th sortname="sum" width="10%">积分</th>	   
			   			<th sortname="p" width="10%">概率</th>	   
			   		</tr> 
			   	</thead>
			   	<tbody> 
